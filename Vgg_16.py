...
def backward(datasets, label, test_data, test_label):
    ...
    #准确率
    accuracy_y = VGG16(X,keep_prob)
    accuracy = tf.equal(tf.argmax(accuracy_y, 1), tf.argmax(Y_, 1))
    ...
    with tf.Session() as sess:
        ...
        for i in range(STEPS):
            ...
            if i % 10 == 0:
                ...
                val_loss = sess.run(loss_mse, feed_dict={X: test_data, Y_: test_label, keep_prob:1})
                train_accuracy = sess.run(accuracy, feed_dict={X: datasets, Y_: label,keep_prob:1})
                test_accuracy = sess.run(accuracy, feed_dict={X: test_data, Y_: test_label,keep_prob:1})
